<template>
    <div class="w-full max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6">
        <h5 class="mb-3 text-base font-semibold text-gray-900 md:text-lg">
            On-going contests
        </h5>
        <ul class="my-4 space-y-3"   v-if="source ? source.length > 0 : null">
            <li v-for="(contest, index) in source">
                <div
                    ...
                    <span class="flex-1 ml-3 whitespace-nowrap text-sm">{{ contest.name }}</span>
                    <RouterLink v-if="role == 'admin'" :to="`contestants/${contest.id}`"
                        class="inline-flex items-center justify-center px-2 py-0.5 ml-3 text-xs font-medium text-white bg-green-500 rounded cursor-pointer">View</RouterLink>
                    <span v-else
