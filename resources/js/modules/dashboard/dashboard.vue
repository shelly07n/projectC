<template>
            ...
            <div class="w-full  max-w-lg p-5 relative mx-auto my-auto rounded-xl shadow-lg  bg-white ">
                ...
                <div class="">
                    ...
                    <div class="p-3  mt-2 text-center space-x-4 md:block">
                        <button @click="useHelper.canShowReferralDialog = false, canHideDialog = false, deactivate = false"
                            class="mb-2 md:mb-0 bg-white px-5 py-2 text-sm shadow-sm font-medium tracking-wider border text-gray-600 rounded-full hover:shadow-lg hover:bg-gray-100">
                            Cancel
                        </button>
                        <button
                            @click="useStore.createContest(referralCode), useHelper.canShowReferralDialog = false, canHideDialog = false, deactivate = false"
    ...
    <!-- <button @click="useStore.createContest('sheltonNereahubMb3O4daton')"
        class="mb-2 md:mb-0 bg-red-500 border border-red-500 px-5 py-2 text-sm shadow-sm font-medium tracking-wider text-white rounded-full hover:shadow-lg hover:bg-red-600">Submit</button> -->
</template>
...
const canHideDialog = ref(false)

const deactivate = ref(true)

const getReferralStatus = async (id) => {
   await axios.get(`/api/showReferralDialog/${id}`).then(res => {
        canHideDialog.value = res.data.canShowReferral
    })
}
...
onMounted(async () => {
    ...
    await getReferralStatus(useHelper.decodedToken.user_id)
})
</script>
